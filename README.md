...
- `post-comment`: Post PR comment with actionable items only (dependency changes + added diagnostics, default: `true`)
...
- The action writes a comprehensive report to the GitHub job summary (`GITHUB_STEP_SUMMARY`) on both PR diff and non-PR snapshot runs.
- PR comments are intentionally actionable-only: dependency changes to review and newly added diagnostics.
- If there are no actionable items, the action skips posting a PR comment and removes prior managed StructuraLens comments for that PR.
- If the actionable PR comment is too large for GitHub comment limits, the action posts a compact summary and uploads the full actionable markdown as artifact `structuralens-actionable-comment.md`.
