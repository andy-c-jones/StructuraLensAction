...
async function downloadCli(version) {
  ...
  const headers = { Authorization: `token ${token}`, Accept: 'application/octet-stream' };
  ...
  const cliName = os.platform() === 'win32' ? 'StructuraLens.Cli.exe' : 'StructuraLens.Cli';
...
async function analyzeWithRefs(cliPath, solution, baseSha, headSha, workspace) {
  ...
  execFileSync('git', ['checkout', '--force', baseSha], { stdio: 'inherit', cwd: repoRoot });
  const baseReport = path.join(baseDir, 'report-base.json');
  runCli(cliPath, ['analyze', solution, '--format', 'json', '--out', baseReport], workspace);

  core.info(`Checking out head ref ${headSha}`);
  execFileSync('git', ['checkout', '--force', headSha], { stdio: 'inherit', cwd: repoRoot });
  const headReport = path.join(headDir, 'report-head.json');
  runCli(cliPath, ['analyze', solution, '--format', 'json', '--out', headReport], workspace);
...
async function main() {
  try {
    ...
    if (isPullRequest && runDiff) {
      ...
      const { baseReport, headReport } = await analyzeWithRefs(cliPath, solution, baseSha, headSha, workdir);
      baseReportPath = baseReport;
      headReportPath = headReport;

      diffReportPath = path.join(workdir, '.structuralens', 'diff.json');
      runCli(cliPath, ['diff', '--base', baseReportPath, '--head', headReportPath, '--format', 'json', '--out', diffReportPath], workdir);

      if (reportHtml) {
        diffHtmlPath = path.join(workdir, '.structuralens', 'diff.html');
        runCli(cliPath, ['diff', '--base', baseReportPath, '--head', headReportPath, '--format', 'html', '--out', diffHtmlPath], workdir);
      }
      ...
      if (postComment) {
        ...
        runCli(cliPath, [
          ...
          '--format', 'markdown',
          '--out', markdownPath,
          '--max-projects', String(maxProjects)
        ], workdir);
      ...
      if (reportJson) {
        ...
        runCli(cliPath, ['analyze', solution, '--format', 'json', '--out', jsonPath], workdir);
        headReportPath = jsonPath;
      }
      if (reportHtml) {
        const htmlPath = path.join(workdir, 'structuralens-report.html');
        runCli(cliPath, ['analyze', solution, '--format', 'html', '--out', htmlPath], workdir);
